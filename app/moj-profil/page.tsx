...
export const metadata: Metadata = {
  title: "Moj profil | Djox Barbershop",
};
...
export default async function MojProfilPage() {
  ...
  // Loyalty prsten (čisto vizuelno — bez logike nagrada).
  const obim = 440;
      ...
      <header className="fixed top-0 z-50 w-full border-b border-border-subtle bg-background/95 backdrop-blur-md">
        <div className="mx-auto flex h-20 max-w-[1280px] items-center justify-between px-gutter">
          ...
          <nav className="hidden items-center gap-8 md:flex">
            ...
            <span className="font-bold text-blood-red">Profil</span>
          </nav>
      ...
      <main className="mx-auto max-w-[1280px] px-gutter pb-24 pt-32">
        ...
        <section className="mb-stack-lg">
          <p className="font-script text-4xl text-blood-red md:text-5xl">
            Dobro došao{user.ime ? `, ${user.ime}` : ""}
          </p>
          <p className="font-body-md text-muted-gray">Spreman za novo osvježenje?</p>
        </section>
        ...
        <div className="grid grid-cols-1 gap-gutter md:grid-cols-12">
          ...
          <div className="relative flex min-h-[300px] flex-col justify-between overflow-hidden rounded-xl border border-border-subtle bg-surface-container p-stack-lg md:col-span-8">
            <div className="relative z-10">
              ...
              {sledeci ? (
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
                  <div>
                    <p className="font-headline-lg text-3xl md:text-5xl">
                      {sledeci.usluga?.naziv ?? "Termin"}
                    </p>
                    <p className="font-body-md text-muted-gray">
                      {formatSalonDatumDugi(sledeci.pocetak)} •{" "}
                      {formatSalonTime(sledeci.pocetak)}h
                    </p>
                  </div>
                ...
                <div>
                  <p className="font-headline-lg text-3xl md:text-4xl">
                    Nemaš zakazanih termina
                  </p>
                  <p className="mt-2 font-body-md text-muted-gray">
                    Vrijeme je za novi rez. Zakaži svoj termin u par klikova.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="flex flex-col items-center justify-center rounded-xl border border-border-subtle bg-surface-container p-stack-lg text-center md:col-span-4">
            ...
            <div className="relative mb-6 flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="font-caption uppercase tracking-widest text-muted-gray">
                  {brojPoseta === 1 ? "posjeta" : "posjeta"}
                </span>
              </div>
            </div>
            <p className="font-body-md text-sm text-on-surface">
              {brojPoseta === 0
                ? "Tvoja prva posjeta tek dolazi!"
                : "Hvala što nam se vraćaš!"}
            </p>
          </div>
          ...
          <div className="rounded-xl border border-border-subtle bg-surface-container p-stack-lg md:col-span-12 lg:col-span-7">
            ...
            <div className="space-y-4">
              {ostali.length === 0 && (
                <p className="text-sm text-muted-gray">
                  {sledeci
                    ? "Nemaš drugih zakazanih termina."
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-gray">
                        {formatSalonDatumKratki(t.pocetak)} •{" "}
                        {formatSalonTime(t.pocetak)}h
                      </p>
                    </div>
                  </div>
                  <OtkaziDugme rezervacijaId={t.id} />
                </div>
              ))}
            </div>
            <Link
            ...
            >
              <span className="material-symbols-outlined">add_circle</span> Zakaži novi
              termin
            </Link>
          </div>
            ...
            <div className="flex items-center gap-6 rounded-xl border border-border-subtle bg-surface-container p-stack-lg">
              ...
              <div>
                ...
                <p className="font-accent-label text-on-surface">
                  {posljednjaPosjeta
                    ? formatSalonDatumDugi(posljednjaPosjeta.pocetak)
                    : "Još nije bilo posjeta"}
                </p>
              </div>
            </div>
            <div className="flex flex-col justify-between rounded-xl border border-border-subtle bg-gradient-to-br from-blood-red/10 to-transparent p-stack-lg">
              <div>
                <h2 className="mb-2 text-sm font-bold uppercase tracking-widest text-on-surface">
                  Spreman za novi look?
                </h2>
                <p className="text-sm text-muted-gray">
                  Izaberi uslugu i slobodan termin — zakazivanje traje manje od minuta.
                </p>
              </div>
              <Link
                href="/zakazivanje"
      ...
      <footer className="border-t border-border-subtle bg-surface-container-lowest py-10">
        <div className="mx-auto max-w-[1280px] px-gutter text-center">
          <p className="text-xs uppercase tracking-widest text-muted-gray">
            © 2026 DJOX BARBERSHOP. Sva prava zadržana.
          </p>
        </div>
      </footer>
    </div>
